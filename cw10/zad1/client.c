  ...
  if(type != 1 && type !=2){
    ...
    while(1)
    {
        ...
        ssize_t rec = recv(sockfd, &serverMsg, sizeof(serverMsg), MSG_WAITALL);
				...
				// if(strcmp("ADD", serverMsg.type) == 0) tmp =0;
				// else  if(strcmp("SUB", serverMsg.type) == 0) tmp =1;
				// else if(strcmp("MUL", serverMsg.type) == 0) tmp = 2;
				// else if(strcmp("DIV", serverMsg.type) == 0) tmp = 3;
				// else if(strcmp("PING", serverMsg.type) == 0) tmp = 4;
